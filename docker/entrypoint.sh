...
if [ -z "${SMTP_HOST:-}" ] || [ -z "${SMTP_PORT:-}" ] || [ -z "${SMTP_USER:-}" ] || [ -z "${SMTP_PASSWORD:-}" ] || [ -z "${SMTP_FROM:-}" ]; then
  echo "SMTP não configurado. Recuperação de senha e envio de convites por e-mail ficarão temporariamente indisponíveis."
fi
