...
export default async function ContractEditorialPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <WorkspacePage eyebrow="Revisão editorial" title="Arquivo-mestre 1.4.0" description="Catálogo sem respostas de usuários. As prévias usam identidades fictícias e não são contratos emitidos.">
    <Alert variant="warning">Produção bloqueada. {report.missingApplicability} perguntas sem aplicabilidade, {report.pendingModules} módulos conjuntos por estruturar e {report.pendingCrossRules} cruzamentos sem predicado.</Alert>
    ...
    <Card className="space-y-4"><Badge>{q.id}</Badge><h2 className="text-2xl font-semibold">{q.title}</h2><p>{q.prompt}</p><p className="text-sm text-muted">{q.period} · {q.applicability ? "Condição de contexto cadastrada" : "Aplicabilidade ainda não definida"}</p>{q.options.map(o => <p key={o.code}><strong>{o.code}.</strong> {o.text}</p>)}
    ...
    <div className="my-6 space-y-4"><h2 className="text-xl font-semibold">Redações conjuntas finalizadas — prévia editorial</h2>{texts.length ? texts.map(c => <Card key={c.id}><p className="mb-3 text-sm text-muted">{c.id}</p><p>{renderRegisteredTemplate(c.editorialTemplate!, { "Nome 1": "Pessoa 1", "Nome 2": "Pessoa 2" })}</p></Card>) : <Alert>Esta pergunta não tem texto no lote de 487 redações conjuntas. Outros componentes permanecem sujeitos à consolidação.</Alert>}</div>
    {catalog.modules.filter(m => m.questionId === q.id).map(m => <Card key={m.id} className="mb-4"><h3 className="font-semibold">{m.id} — {m.compiled ? "Definição compilada; ativação pendente" : "Módulo indisponível"}</h3>{m.compiled ? <><p className="my-3">{m.prompt}</p>{m.options.map(o => <div className="mb-4" key={o.code}><p>{o.code}. {o.text}</p><p className="mt-2 text-sm text-muted">{o.template}</p></div>)}</> : <p className="mt-2 text-sm text-muted">{m.status}</p>}</Card>)}
    <Card><h2 className="text-xl font-semibold">Decisões restantes</h2><ul className="mt-4 list-disc space-y-2 pl-5"><li>Aplicabilidade das 184 perguntas e mapeamento final das cláusulas.</li><li>Campos, opções e textos dos outros módulos; predicados dos 96 cruzamentos.</li><li>Revisão privada, retomada segura, retenção e exclusão de dados.</li><li>Escopo dos agregados de segurança e tratamento de inaplicabilidade.</li><li>Compra, prazo de acesso, cancelamento e reembolso.</li><li>Eventos, prazos, parâmetros abertos e Registro Familiar de Emergência.</li><li>Referências bíblicas e forma de aceite do instrumento.</li></ul><p className="mt-4">A redação dos 487 textos está encerrada. Eles aguardam integração funcional, sem necessidade de nova decisão editorial.</p></Card>
