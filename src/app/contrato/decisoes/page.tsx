...
import { JointContext } from "@/features/contract/components/joint-context";
...
export default async function ContractDecisionsPage() {
  ...
  return <div className="space-y-6"><h2 className="font-serif text-3xl text-brand-strong">NÓS DECIDIMOS</h2><Alert>{shared.message}</Alert><ContractNextSteps />{shared.facts ? <JointContext key={shared.facts.revision} facts={shared.facts} /> : null}<JointDecisions modules={shared.modules} /></div>;
}
